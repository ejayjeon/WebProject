...
const getData = ({ path, method, data, callback }) => {
    ...
    fetch(`${Host}${path}`, {
        ...
        headers: {
            ...
            'auth-key': sessionStorage.getItem('key')
        },
        ...
        .then((d) => {
            if(d.result === 'fatal' && d.err === 'access denied'){
                window.alert('새로운 연결이 확인 되었습니다. 로그아웃 합니다.')
...
export const reqSMSAuthCode = ({ country_dial, phone, uuid, callback }) => {
    getData({
        path: '/users/sms-auth',
        data: {
            country_dial, phone, uuid
        },
        callback
    })
}
export const reqSMSAuthCode2 = ({ country_dial, phone, uuid, callback }) => {
    getData({
        path: '/users/sms-auth2',
        data: {
            country_dial, phone, uuid
        },
        callback
...
export const readBlockState = ({ callback }) => {
    getData({
        path: '/map/block-state/read',
        callback
    })
}
export const readPopupData = ({ callback }) => {
    getData({
        path: '/popup/read-data',
        callback
    })
}
export const readTemporaryData = ({ callback }) => {
    getData({
        path: '/temporary-data/read',
        callback
    })
}
export const checkCPComplete = ({ email, password, callback }) => {
    getData({
        path: '/nice-id/checked_user',
        data: {
            type: 'reqMemberInfo',
            data: {
                kind: 'ID',
...
export const checkPhoneNumberDuplicate = ({ phone, country_dial, callback }) => {
    getData({
        path: '/users/phone/duplicate',
        data: {
            type: 'reqMemberInfo',
            data: {
                kind: 'PHONE',
...
export const addEventTargetUser = ({ swId, phone, email, birthday, gender, nationinfo, mobileco, name, callback }) => {
    getData({
        path: '/users/event-target-user/add',
        data: {
            swId, phone, email, birthday, gender, nationinfo, mobileco, name
        },
        callback
    })
}
export const checkMember = ({ email, password, callback }) => {
    getData({
        path: '/users/check-member',
        data: {
            type: 'reqLogin',
            data: { email, password }
        },
