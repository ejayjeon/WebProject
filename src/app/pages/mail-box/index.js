...
const TableItem = ({ index, type, title, date, sender, onClickHandler }) => {
    return (
        ...
        >
            {/* 정렬 */}
            <div className={Style['msgLineUp']} style={{ width: '10%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>{index}</div>
            <div className={Style['msgTitle']} style={{ width: '50%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>{title}</div>
            <div className={Style['msgDate']} style={{ width: '30%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <NumberFormat format="####/##/## ##:##:##" placeholder="YYYY/MM/DD HH/mm/ss" mask={['Y', 'Y', 'Y', 'Y', 'M', 'M', 'D', 'D', 'H', 'H', 'm', 'm', 'S', 'S']}
                value={date}
                displayType="text"/>
                </div>
            <div className={Style['msgSender']} style={{ width: '10%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>{sender}</div>
        </div>
    )
}
...
const TableHeader = () => {
    return (
        ...
        >
            <div className={Style['tableHeader1']} 
            style={{ width: '10%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}>순서</div>
            <div className={Style['tableHeader2']}
            style={{ width: '50%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}>제목</div>
            <div className={Style['tableHeader3']}
            style={{ width: '30%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}>날짜 ( UTC ) </div>
            <div className={Style['tableHeader4']}
            style={{ width: '10%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}>보낸이</div>
        </div>
    )
}
const ViewDetail = ({ title, sender, registerTime, msg, onCloseHandler, msgType, msgRef = '', showDetail, userUUID }) => {
    ...
    return (
        ...
        >
            <div   
            style={{ width: '600px', height: 'calc(100% - 200px)', backgroundColor: 'white', border: '1px solid #e17a18' }}>
