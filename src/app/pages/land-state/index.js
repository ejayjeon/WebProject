...
import {
  ...
  purchaseProposal,
  updateMyLandState,
  // getAuctionClose, getMyLandList, getOwnedLandList,
} from "../../api";
import {
  // getPointAmount,
  // applyAuction,
...
const onClickHandler = (e) => {
  if (mouseAction.leftButtonDown && mouseAction.mouseMove) {
    ...
    !clickIsBlocked &&
    grids[`${e.gridX}-${e.gridY}`].available
  ) {
    clickIsBlocked = true;
    ...
    if (!alreadySetStartPoint()) {
      if (selectedPoint.gridX !== e.gridX || selectedPoint.gridY !== e.gridY) {
        if (selectedPoint.gridX && selectedPoint.gridY) {
          grids[`${selectedPoint.gridX}-${selectedPoint.gridY}`].setSelected(
            false
          );
        }
...
const MyLandState = ({ userUUID }) => {
  const [forSale, setForSale] = useState(
    grids[`${selectedPoint.gridX}-${selectedPoint.gridY}`].forSale
  );
  const [salePrice, setSalePrice] = useState(
    grids[`${selectedPoint.gridX}-${selectedPoint.gridY}`].salePrice
  );
  return (
    <React.Fragment>
      <div
        ...
        <div
          ...
          onClick={() => {
            updateMyLandState({
              ...
              salePrice: salePrice,
              gridId:
                grids[`${selectedPoint.gridX}-${selectedPoint.gridY}`].tbIndex,
              callback: (err, response) => {
                if (err) {
                  console.log(err);
                } else {
                  if (response.result === "success") {
                    let forSale =
                      response.myLand.forSale === "yes" ? true : false;
                    grids[
                      `${selectedPoint.gridX}-${selectedPoint.gridY}`
                    ].forSale = forSale;
                    grids[
                      `${selectedPoint.gridX}-${selectedPoint.gridY}`
                    ].setForSales(forSale);
                    grids[
                      `${selectedPoint.gridX}-${selectedPoint.gridY}`
                    ].salePrice = response.myLand.salePrice;
              ...
              <React.Fragment>
                ...
                >
                  <div className={classNames(Style["info-box"])}>
                    {grids[`${selectedPoint.gridX}-${selectedPoint.gridY}`]
                      .myLand === 0 &&
                    !grids[`${selectedPoint.gridX}-${selectedPoint.gridY}`]
                      .others ? (
              ...
              getMapBlockData({
                ...
                callback: (err, res) => {
                  if (err) {
                    ...
                    gridBackground.changeMap(
                      `/images/map-sp-${blockX}${blockY}.jpeg`
                    );
                    setLoading(false);
                    setShowSPMap(true);
                  }
            ...
            getMapBlockData({
              ...
              callback: (err, res) => {
                if (err) {
                  ...
                  gridBackground.changeMap(
                    `/images/map-sp-${data.blockX}${data.blockY}.jpeg`
                  );
  ...
  return (
    <React.Fragment>
      <div className={classNames(Style["background"])}>
        ...
        />
        <div
          style={{ backgroundColor: "#33a4ef", position: "relative" }}
