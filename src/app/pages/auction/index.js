  ...
  if (mouseAction.leftButtonDown && mouseAction.mouseMove) {
    ...
    if (!alreadySetStartPoint()) {
      ...
      if (maxSelectedGrid < rowCount * columnCount) {
        window.alert(
          `선택 가능한 그리드 최대 개수는 ${maxSelectedGrid} 개입니다.`
        );
        clickIsBlocked = false;
        document.body.style.cursor = "default";
        return;
        ...
        {!showSPMap ? undefined : selectedArea.exist ? (
                                ...
                                if (result === true) {
                                  if (
                                    ...
                                    if (
                                      ...
                                      window.alert(
                                        `한번에 참여 할 수 있는 최대 Land의 개수는 ${maxSelectedGrid} 개 입니다.`
                                      );
                                      return;
                                    }
                                    console.log(
                                      ...
                                      callback: (err, res) => {
                                        if (err) {
                                          ...
                                          if (
                                            ...
                                            window.alert(
                                              "모든 경매가 종료되었습니다. 더이상 경매에 참여 할  수 없습니다."
                                            );
                                            initSelectPoint();
                                            setShowDetail(false);
                                            setSelectedArea({ exist: false });
                                              ...
                                              if (err) {
                                                ...
                                                if (
                                                  ...
                                                  if (point >= totalBidPrice) {
                                                      ...
                                                      callback: (err, res) => {
                                                        if (err) {
                                                          ...
                                                          if (allClear) {
                                                            ...
                                                            setLoading(false);
                                                            window.alert(
                                                              `경매 입찰에 실패 하였습니다. 경매가가 상승하였습니다. 다시 시도해 주세요`
                                                            );
                                                          }
                                                        }
                                                      },
                                                    ...
                                                    setLoading(false);
                                                    window.alert(
                                                      "경매 입찰에 실패하였습니다. 현재 보유한 포인트가 Land 가격보다 적습니다."
                                                    );
                                                  }
                                                }
                                              }
          ...
          <>
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: language["auction-map-14"], // 선택된 영역이 없습니다.
              }}
            ></div>
            <ColorInfo language={language} />
          </>
      ...
      onClick={() => {
        ...
        if (blockState[blockY][blockX].blockState === 1) {
          ...
          if (blockState[blockY][blockX].blockState === 3) {
            if (
              !window.confirm(
                "해당 블록은 경매를 모두 종료하고 일반 지도가 오픈 되었습니다. 일반 지도로 이동하시겠습니까?"
              )
            ) {
              return;
            }
            ...
            //window.alert('해당 블록은 아직 열리지 않았습니다.')
          } else if (blockState[blockY][blockX].blockState === 2) {
            window.alert(
              "해당 블록은 모든 경매를 종료하고 일반 지도 오픈을 기다리고 있습니다."
            );
          }
        }
      }}
